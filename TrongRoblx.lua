...
object.TrongRoblox = instance.new("ScreenGui", {
    Name = "Mo Mo Be";
    Parent = game.CoreGui;
...
pcall(function()
    if true and isfile('TrongRoblox.bin') and not getgenv().Key then
        object.TextBox.Text = readfile("TrongRoblox.bin")
...
local function SaveKey()
    local s, e = pcall(function()
        if not isfile('TrongRoblox.bin') then
            writefile('TrongRoblox.bin', GetKeyText())
        end
    end)
end
...
local function RemoveKey()
    local s, e = pcall(function()
        if isfile('TrongRoblox.bin') then
            delfile('TrongRoblox.bin')
        end
    end)
end
    ...
    if sign_match == decode_response.sign then
        ...
        function Window:NewTab(mmb)
            ...
            function newmm:NewParagraph(cf)
                local paragraphnew
                newTitle = cf.Title or "Trong Roblox"
                ...
                function paragraphnew:Set(cf2)
                    newTitle = cf2.Title or "Trong Roblox"
        ...
        function SaveSettings2()
            local HttpService = game:GetService("HttpService")
            if not isfolder("Trong Roblox") then
                makefolder("Trong Roblox")
            end
            writefile("Trong Roblox/" .. SaveFileName2, HttpService:JSONEncode(Settings2))
        end
        ...
        function ReadSetting2()
            local s, e =
                ...
                function()
                    ...
                    return HttpService:JSONDecode(readfile("Trong Roblox/" .. SaveFileName2))
                end
            )
            if s then
                return e
